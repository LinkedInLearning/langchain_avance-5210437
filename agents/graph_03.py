...
class MyGraph:
    ...
    def __init__(self): 
        ...
        agent_graph.add_edge("Node2","Node4")
        agent_graph.add_edge("Node4","Node6")
        agent_graph.add_edge("Node6",END) 
        agent_graph.add_edge("Node3","Node5")
        agent_graph.add_edge("Node5",END)
 
        #agent_graph.add_edge("Node5","Node1")
    ...
    def my_condition(self, state:OrdersAgentState):
        rnd = random.randint(0, 10)
        print(f"the random number from conditional edge: {rnd}")
        if rnd > 5 : 
            return True
        return False
